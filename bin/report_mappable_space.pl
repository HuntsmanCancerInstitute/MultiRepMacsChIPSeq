...
{
	my $sam = open_db_connection($example, 1) or 
		die "unable to read bam file '$example'!\n";
...
foreach my $chr (keys %chr_list) {
	...
	my $unique_set = Set::IntSpan::Fast->new() or 
		die "unable to initialize Integer Set! $!\n";
	...
	foreach my $file (@bamfiles) {
		...
		my $sam = open_db_connection($file, 1) or 
			die "unable to open bam file '$file'!\n";
		
		# check chromosome and length
		my ($tid, undef, undef) = $sam->header->parse_region($chr);
		my $seq_length = $sam->target_len($tid);
		if ($length != $seq_length) {
			warn " ! Sequence length discrepancy for $chr in $file! Compare $length with $seq_length\n";
		}
