...
my %chip2group;
if ($groupfile) {
	my $Groups = Bio::ToolBox->load_file($groupfile) or 
		die " unable to load group file '$groupfile'!\n";
	$Groups->iterate( sub {
		my $row = shift;
...
sub collect_peak_data {
	...
	my $db = Bio::ToolBox->open_database($file) or 
		warn " Invalid ChIP file specified! Can't open $file!\n";
	return unless ($db);
	...
	foreach my $chr (keys %$chromhash) {
		...
		while (my $intervals = $iter->next) {
			foreach my $interval (@$intervals) {
				...
				my $results = exists $tree{$chr} ? $tree{$chr}->fetch($i_start, $i_end) : [];
	...
	return [$name, $group, format_with_commas($on_peak_sum), 
