...
my $GeneData         = Bio::ToolBox->new_data(
	qw(GeneID GeneName Overlapping Closest
		Adjacent Neighborhood)
);
my %profile =
	map { $_ => 0 } ( -1 * downsize($profile_radius) .. downsize($profile_radius) );
...
sub load_tss_data {
	...
	if ( not $tss_file and $anno_file ) {
		...
		system($cmd) == 0
			or die " FATAL: execution '$cmd' failed!\n";
	}
	...
	$Data->iterate(
		sub {
			...
			$id2alt{ $row->value($trxid) } =
				[ $row->value($genid), $row->value($gname) ];
