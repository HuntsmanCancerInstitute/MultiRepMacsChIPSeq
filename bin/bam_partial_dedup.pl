...
$htext .= sprintf( "\@PG\tID:bam_partial_dedup\tVN:%s\tCL:%s", $VERSION, $PROGRAM_NAME );
$htext .= ' --pe'                                  if $paired;
...
"\n Something is wrong! Zero $items were counted!\n Check your bam file to make sure mapped alignments are present.\n";
		if ($paired) {
			print
" Also check that the bam file truly has paired-end alignments,\n or run in single-end mode (without --pe option)\n\n";
		}
		exit;
	}
  ...
  Mean position depth: %17.4f\n",
	...
	if ($coordErrorCount) {
		printf
"  WARNING: %d Alignments did not have read name pixel coordinates for optical detection!\n",
			$coordErrorCount;
...
"# number is duplicate count observed at X and Y delta distance from previous\n"
		...
		$fh->close;
		print
"  wrote table of duplicate fraction at each pixel distance to file $histogramfile\n";
	}
	...
	if ( $fraction and $dupRate <= $fraction ) {
		...
		print
" Actual duplication rate is less than indicated target fraction ($fraction).\n";
		...
		if ( defined $max and $max >= 1 and $do_optical ) {
			...
			if ( $maxObserved <= $max ) {
				...
				if ( $opticalRate > $max_optical_rate ) {
					print
" Optical duplicate rate is higher than allowed ($max_optical_rate).\n";
			...
			if ( $maxObserved <= $max ) {
				print
" Maximum observed depth is less than maximum allowed ($max).\n No de-duplication necessary.\n";
...
sub count_alignments_singlethread {
	...
	return (
		$totalCount, $opticalCount, $coordErrorCount, \%depth2count,
...
sub count_up_pe_alignments {
	...
	foreach my $s ( keys %sizes ) {
		...
		if ($do_optical) {
			my ( $nondup, $dup, $coord_error ) =
				identify_optical_duplicates( $sizes{$s}, $data );
			$data->{depth2count}{ scalar @{$nondup} } += 1;
			$data->{optCount}                         += scalar @{$dup};
			$data->{coordError}                       += $coord_error;
		}
...
sub identify_optical_duplicates {
	...
	foreach my $a ( @{$alignments} ) {
		...
		if (
			$#bits < $ypos
			or (    not defined $bits[$tilepos]
				and not defined $bits[$xpos]
				and not defined $bits[$ypos] )
			)
		{
	...
	while ( my ( $rgtile, $rgtile_alnts ) = each %tile2alignment ) {
		...
		if ( scalar @{$rgtile_alnts} == 1 ) {
			...
			if ($report_distance) {
				...
				my @ydiffs = map { abs( $spots[$_]->[1] - $spots[ $_ - 1 ]->[1] ) }
					( 1 .. $#spots );
				for my $i ( 0 .. $#xdiffs ) {
			...
			if ( min(@xdiffs) > $optical_thresh ) {
				...
				while (@spots) {
					if ( $spots[0]->[0] - $first->[0] < $optical_thresh ) {
						...
						while ( scalar @spots
							and $spots[0]->[0] - $closest[-1]->[0] < $optical_thresh )
						{
						...
						my @ydiffs = map { $closest[$_]->[1] - $closest[ $_ - 1 ]->[1] }
							( 1 .. $#closest );
							...
							while (@closest) {
								if ( $closest[0]->[1] - $xyfirst->[1] < $optical_thresh )
								{
								  ...
								  # continue compareing the next one with the previous one
									while ( scalar @closest
										and $closest[0]->[1] - $clustered[-1]->[1] <
										$optical_thresh )
									{
...
sub deduplicate_singlethread {
	...
	$outbam = Bio::ToolBox::db_helper::write_new_bam_file($outfile)
		or die "unable to open output bam file $outfile! $OS_ERROR";
...
sub deduplicate_multithread {
	...
	for my $tid (@tid_list) {
		...
		my $tempbam = Bio::ToolBox::db_helper::write_new_bam_file($tf)
			or die "unable to open output bam file $tf! $OS_ERROR";
...
sub write_out_max_pe_alignments {
	...
	foreach my $s ( sort { $a <=> $b } keys %f_sizes ) {
		...
		if ($do_optical) {
			my ( $nondup, $dup, $coord_error ) =
				identify_optical_duplicates( $f_sizes{$s} );
			@reads = @{$nondup};
			$data->{optical} += scalar @{$dup};
			if ( $keep_optical and scalar( @{$dup} ) ) {
...
sub write_out_random_se_alignments {
	...
	foreach my $pos ( keys %fends ) {
		...
		if ($do_optical) {
			my ( $nondup, $dup, $coord_error ) =
				identify_optical_duplicates( $fends{$pos} );
			@reads = @{$nondup};
			$data->{optical} += scalar @{$dup};
			if ( $keep_optical and scalar( @{$dup} ) ) {
	...
	foreach my $pos ( keys %rends ) {
		...
		if ($do_optical) {
			my ( $nondup, $dup, $coord_error ) =
				identify_optical_duplicates( $rends{$pos} );
			@reads = @{$nondup};
			$data->{optical} += scalar @{$dup};
			if ( $keep_optical and scalar( @{$dup} ) ) {
...
sub write_out_random_pe_alignments {
	...
	foreach my $s ( sort { $a <=> $b } keys %f_sizes ) {
		...
		if ( scalar @reads == 1 ) {
			...
			if ( defined $max and scalar(@keep) >= $max ) {
				push @toss,
					splice( @keep, $max - 1 );    # we've already written the first one
			}
