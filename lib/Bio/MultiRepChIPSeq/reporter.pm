...
Fragment coverage tracks were generated from $status alignments, 
ignoring secondary, supplementary, marked duplicate alignments, with a mapping 
quality greater than `$map`. Alignments overlapping exclusion intervals or on 
excluded chromosomes were skipped.
END
...
END
		...
		$string  .= <<END;
Fragment coverage was generated by shifting the alignment start coordinate by $shift bp
and then extending by $size bp. 
END
	}
	else {
		my $size = $self->fragsize;
...
threshold of $cutoff (essentially FDR <= $decimal), minimum peak size of $peaksize bp,
and a maximum gap of $peakgap bp. The following number of peaks were identified.
...
Broad, gapped peaks were called independently for each sample replicate. Dispersed or
low-abundance peaks were called with a cutoff of $broadcut (essentially FDR <=
$decimal2). Significant peaks were called using a q-value threshold of $cutoff
(essentially FDR <= $decimal) and maximum gap of $peakgap bp. The maximum linking
between significant peaks was $broadgap bp. The minimum minimum peak length of
...
These peaks were then merged into a final master replicate-merge peak set for combined
sample comparison, file `$merged_peak`, with $merged_count intervals.
END
...
Independently called replicate gapped peaks were merged for each sample, requiring at least
$overlap overlaps and a maximum gap of $gap bp. The final number of peaks for each
sample are below.
...
These peaks were then merged into a final master broad replicate-merge peak set for combined
sample comparison, file `$merged_peak`, with $merged_count intervals.
END
...
These peaks were then merged into a final master replicate-mean peak set for combined
sample comparison, file `$mean_peak`, with $mean_count intervals.
END
...
coverage and enrichment tracks. Dispersed or low-abundance peaks were called with a
cutoff of $broadcut (essentially FDR <= $decimal2). Significant peaks were called
using a q-value threshold of $cutoff (essentially FDR <= $decimal) and maximum gap of
$peakgap bp. The maximum linking between significant peaks was $broadgap bp. The
minimum minimum peak length of $peaksize bp.
...
These peaks were then merged into a final master broad replicate-mean peak set for
combined sample comparison, file `$mean_peak`, with $mean_count intervals.
END
